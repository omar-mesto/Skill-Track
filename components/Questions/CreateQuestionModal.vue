...
import { useCreateQuestion } from '@@/queries/questions/index'
...
const onImageChange = (e: Event) => {
  ...
  questionImage.value = target.files[0]
}
...
const submit = async () => {
  ...
  if (questionImage.value) fd.append('questionImage', questionImage.value)

  const { execute, status, data } = useCreateQuestion(fd)
  await execute()

  if (status.value === 'success' && data.value?.data) {
    toast.add({ description: 'Question Created', color: 'success', class: 'text-black bg-white' })
    ...
    toast.add({ description: errorStore.message || 'Error', color: 'error', class: 'text-black bg-white' })
  }
