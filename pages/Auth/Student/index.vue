<script lang="ts" setup>
import { useLoginStudent } from '@@/queries/Auth/student'
import type { loginModel } from '@@/models/loginModel'
import type { LoginResponse } from '@@/models/loginResponseModel'

const router = useRouter()

const studentLoginFunction = async (userData: loginModel) => {
  const { data, status } = await useLoginStudent(userData)
  if (status.value === 'success') {
    const loginData = data.value as LoginResponse
    router.push('../../Profile/Student')

    return { success: true, role: loginData.data.user.role, id: loginData.data.user.id, token: loginData.data.token, name: loginData.data.user.name, email: loginData.data.user.email }
  } else {
    return { success: false }
  }
}
</script>

<template>
  <div class="w-full bg-white text-black h-dvh flex flex-col md:flex-row">
    <Auth :login-function="studentLoginFunction">
      <p class="text-lg md:text-3xl font-bold text-black mt-16">
        Student Login
      </p>
      <p class="text-[#868686] pt-3">
        Enter your Email and Password
      </p>
      <template #button>
        <NuxtLink class="font-bold cursor-pointer text-primary hover:underline">
          Sign in
        </NuxtLink>
      </template>
      <template #Register>
        <p
          class="text-center pt-5"
        >
          You Do Not Have Account?
          <span
            class="text-Sequand underline mx-1 cursor-pointer font-bold"
            @click="navigateTo('/Auth/Student/register')"
          >Sign Up</span>
        </p>
      </template>
    </Auth>
    <div class="w-1/2 bg-primary md:grid hidden h-screen place-items-center">
      <img
        src="../../../public/StudentLogin.png"
        loading="lazy"
        class="w-full md:w-xl"
      >
    </div>
  </div>
</template>
