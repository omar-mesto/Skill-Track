...
import { useRegisterProfessor } from '@@/queries/Auth/professor'
import { registerProfessorSchema } from '@@/schema/registerProfessor'
import type { RegisterResponse } from '~/models/registerModel'
import { UTextarea } from '#components'

const router = useRouter()

const professorFrom = ref({
  name: '',
  email: '',
  password: '',
...
const registerUser = async () => {
  ...
  formData.append('name', professorFrom.value.name)
  formData.append('email', professorFrom.value.email)
  formData.append('password', professorFrom.value.password)
  formData.append('bio', professorFrom.value.bio)
      ...
      <div class="flex w-full flex-col items-center px-6">
        ...
        <div class="w-full md:px-1 lg:px-20">
          <div class="md:p-6 md:space-y-6 sm:p-8">
            <UForm
              ref="form"
              :schema="registerProfessorSchema"
              :state="professorFrom"
            ...
            >
              <div class="flex justify-between">
                ...
                >
                  <UInput
                    v-model="professorFrom.specialization"
                    class="border-0 rounded-lg bg-white text-black block"
                    size="xl"
                    variant="subtle"
                  />
              ...
              <div class="flex my-2 justify-between">
                ...
                >
                  <UInput
                    v-model="professorFrom.password"
                    type="password"
                    class="border-0 rounded-lg bg-white text-black block"
                    size="xl"
                    variant="subtle"
              ...
              <p class="text-center mt-5">
                ...
                <span
                  ...
                  @click="navigateTo('/Auth/Professor')"
                >
                  Sign in
                </span>
              </p>
